...
public class HfengScreen extends Screen implements KeyListener {
	...
	// Temp pseudo item pricing
	ArrayList<Integer> itemCost_Kathy = new ArrayList<Integer>();
	ArrayList<Integer> itemCost_Rebecca = new ArrayList<Integer>();
	ArrayList<Integer> itemCost_Inventory = new ArrayList<Integer>();
	...
	public static final double RESALE_VALUE = 0.75;
	...
	public HfengScreen(Game game){
		...
		for(int i = 0; i < NUMBER_OF_POTIONS; i++){
			...
			// pseduo pricing
			itemCost_Kathy.add((int) ((Math.random()*100000)+5000));
		}
		for(int i = 0; i < NUMBER_OF_EQUIPMENTS; i++){
			storeInventory_Rebecca.add(equipStock.equipStocks(i));
			// pseduo pricing
			itemCost_Rebecca.add((int) ((Math.random()*100000)+5000));
		}
				...
				if(BUY_MENU == true){
					...
					if(BUY_WARNING == true){
						...
						if(ITEM_NUMBER <= storeInventory_Kathy.size()){
							...
							g2.drawString(storeInventory_Kathy.get(ITEM_NUMBER-1).getName() + " costs " + NumberFormat.getIntegerInstance().format(itemCost_Kathy.get(ITEM_NUMBER-1)) + " gold coins.", 200, 200);
							g2.setColor(Color.black);
				...
				if(SELL_MENU == true){
					...
					if(SELL_WARNING == true){
						...
						if(ITEM_NUMBER <= playerInventory.size()){
							...
							g2.setColor(GOLD);
							g2.setFont(new Font("Comic Sans", Font.PLAIN, 25));
							g2.drawString(playerInventory.get(ITEM_NUMBER-1).getName() + " sells for " + NumberFormat.getIntegerInstance().format(itemCost_Inventory.get(ITEM_NUMBER-1)* RESALE_VALUE) + " gold coins.", 200, 200);
							g2.setColor(Color.black);
							g2.setFont(new Font("Comic Sans", Font.BOLD, 30));
				...
				if(BUY_MENU == true){
					...
					if(BUY_WARNING == true){
						...
						if(ITEM_NUMBER <= storeInventory_Rebecca.size()){
							...
							g2.setColor(GOLD);
							g2.setFont(new Font("Comic Sans", Font.PLAIN, 25));
							g2.drawString(storeInventory_Rebecca.get(ITEM_NUMBER-1).getName() + " costs " + NumberFormat.getIntegerInstance().format(itemCost_Rebecca.get(ITEM_NUMBER-1)) + " gold coins.", 200, 200);
							g2.setColor(Color.black);
							g2.setFont(new Font("Comic Sans", Font.BOLD, 30));
							g2.drawString("Do you want to buy this?", 200, 250);
							g2.setFont(new Font("Comic Sans", Font.BOLD, 20));
			...
			if(SHOP_1_OPEN == true){
				if(BUY_MENU == true && BUY_WARNING == true){
					if(storeInventory_Kathy.size() > 0){
						if(itemCost_Kathy.get(ITEM_NUMBER-1) < PLAYER_WEALTH){
							playerInventory.add(storeInventory_Kathy.get(ITEM_NUMBER-1));
							PLAYER_WEALTH = PLAYER_WEALTH - itemCost_Kathy.get(ITEM_NUMBER-1);
							storeInventory_Kathy.remove(ITEM_NUMBER-1);
							itemCost_Inventory.add(itemCost_Kathy.get(ITEM_NUMBER-1));
							itemCost_Kathy.remove(ITEM_NUMBER-1);
							if(END % 9 != 0){
								END--;
							}
							BUY_WARNING = false;
				...
				if(SELL_MENU == true && SELL_WARNING == true){
					...
					PLAYER_WEALTH = PLAYER_WEALTH + (int) (itemCost_Inventory.get(ITEM_NUMBER-1) * RESALE_VALUE);
					itemCost_Kathy.add(itemCost_Inventory.get(ITEM_NUMBER-1));
					itemCost_Inventory.remove(ITEM_NUMBER-1);
					if(END % 9 != 0){
						END--;
					}
					SELL_WARNING = false;
			...
			if(SHOP_2_OPEN == true){
				if(BUY_MENU == true && BUY_WARNING == true){
					if(storeInventory_Rebecca.size() > 0){
						if(itemCost_Rebecca.get(ITEM_NUMBER-1) < PLAYER_WEALTH){
							playerInventory.add(storeInventory_Rebecca.get(ITEM_NUMBER-1));
							PLAYER_WEALTH = PLAYER_WEALTH - itemCost_Rebecca.get(ITEM_NUMBER-1);
							itemCost_Inventory.add(itemCost_Rebecca.get(ITEM_NUMBER-1));
							storeInventory_Rebecca.remove(ITEM_NUMBER-1);
							itemCost_Rebecca.remove(ITEM_NUMBER-1);
							if(END % 9 != 0){
								END--;
							}
							BUY_WARNING = false;
				...
				if(SELL_MENU == true && SELL_WARNING == true){
					...
					playerInventory.remove(ITEM_NUMBER-1);
					PLAYER_WEALTH = PLAYER_WEALTH + (int) (itemCost_Inventory.get(ITEM_NUMBER-1) * RESALE_VALUE);
					itemCost_Rebecca.add(itemCost_Inventory.get(ITEM_NUMBER-1));
					itemCost_Inventory.remove(ITEM_NUMBER-1);
					if(END % 9 != 0){
						END--;
					}
